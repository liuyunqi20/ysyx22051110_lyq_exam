...
class CacheStage2(config: CacheConfig) extends Module{
    ...
    val muxSeq = for( i <- 0 until config.nr_ways) yield (target_way1H(i) -> rd_lines(i))
...
class CacheStage3(config: CacheConfig) extends Module with HasCacheStage3Const{
    ...
    io.mem_out.req.bits.wr       := state(1) || ((state(4) & buf.wr) === 1.U)
    io.mem_out.req.bits.addr     := Mux(state(0), wb_addr, cpu_req_addr)
    io.mem_out.req.bits.wdata    := buf.target_line.data(cnt)
